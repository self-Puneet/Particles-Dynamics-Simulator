...
def create_basic_gui():
    root = tk.Tk()
    root.title("Particle Simulation GUI")

    label = ttk.Label(root, text="Welcome to Particle Simulation")
    label.pack(pady=10)

    button = ttk.Button(root, text="Start Simulation", command=create_particle_input_gui)
    button.pack(pady=10)

    root.mainloop()

def create_particle_input_gui():
    root = tk.Tk()
    root.title("Particle Simulation GUI")

    particles_label = ttk.Label(root, text="Enter the number of particles:")
    particles_label.pack(pady=10)

    particles_entry = ttk.Entry(root)
    particles_entry.pack(pady=10)

    button = ttk.Button(root, text="Next", command=lambda: create_time_input_gui(int(particles_entry.get())))
    button.pack(pady=10)

    root.mainloop()

def create_time_input_gui(num_particles):
    root = tk.Tk()
    root.title("Particle Simulation GUI")

    time_label = ttk.Label(root, text="Enter the desired time:")
    time_label.pack(pady=10)

    time_entry = ttk.Entry(root)
    time_entry.pack(pady=10)

    button = ttk.Button(root, text="Next", command=lambda: create_simulation_gui(num_particles, float(time_entry.get())))
    button.pack(pady=10)

    root.mainloop()

def create_simulation_gui(num_particles, desired_time):
    root = tk.Tk()
    root.title("Particle Simulation GUI")

    label = ttk.Label(root, text="Simulation in progress...")
    label.pack(pady=10)
    ...
    net_gravitational_force = np.zeros((t2 + 1, n))
    net_force = np.zeros((t2 + 1, n))

    # Speed and linear momentum arrays
